	...
	if ( ! class_exists( 'EA_WC_Amazon_S3' ) ):
		...
		class EA_WC_Amazon_S3 {
			...
			public function init() {
				load_plugin_textdomain( 'ea-wc-amazon-s3', FALSE, plugin_basename( dirname( __FILE__ ) ) . '/languages' );
			}
			...
			private function hooks() {
				...
				add_action( 'woocommerce_settings_tabs_ea-wc-amazon-s3', array( $this, 'settings_fields' ) );

				// Save Settings Fields
				add_action( 'woocommerce_update_options_ea-wc-amazon-s3', array( $this, 'update_settings' ) );
			...
			public function upload_handler() {
				...
				if ( ! isset( $_POST[ '_wpnonce' ] ) || ! wp_verify_nonce( $_POST[ '_wpnonce' ], 'ea_wc_amazon_s3_upload_file' ) ) {
					wp_die( esc_html__( 'Not Verified', 'ea-wc-amazon-s3' ), esc_html__( 'Error', 'ea-wc-amazon-s3' ), array( 'back_link' => TRUE ) );
				}

				if ( empty( $_FILES[ 'ea_wc_amazon_s3_file' ] ) || empty( $_FILES[ 'ea_wc_amazon_s3_file' ][ 'name' ] ) ) {
					wp_die( esc_html__( 'Please select a file to upload', 'ea-wc-amazon-s3' ), esc_html__( 'Error', 'ea-wc-amazon-s3' ), array( 'back_link' => TRUE ) );
				}
				...
				if ( $this->upload_file( $file ) ) {
					...
					wp_die( esc_html__( 'Something went wrong during the upload process.', 'ea-wc-amazon-s3' ), esc_html__( 'Error', 'ea-wc-amazon-s3' ), array( 'back_link' => TRUE ) );
				}
			}
				...
				if ( is_plugin_active( plugin_basename( __FILE__ ) ) ) {
					$action_links = apply_filters( 'ea_wc_amazon_s3_action_links', array(
						'settings' => sprintf( '<a href="%1$s" title="%2$s">%2$s</a>', esc_url( add_query_arg( array(
							                                                                                       'page' => 'wc-settings',
							                                                                                       'tab'  => 'ea-wc-amazon-s3'
						                                                                                       ), admin_url( 'admin.php' ) ) ), esc_attr__( 'Amazon S3 Settings', 'ea-wc-amazon-s3' ) ),
					) );
			...
			public function admin_enqueue_scripts() {
				wp_enqueue_media();
				wp_enqueue_script( 'ea-wc-amazon-s3', untrailingslashit( plugin_dir_url( __FILE__ ) ) . '/js/scripts.js', array( 'jquery' ), FALSE, TRUE );
			}
			...
			public function buckets_iframe_content( $type = 'file', $errors = NULL, $id = NULL ) {
				...
				<div style="margin: 20px 1em 1em; padding-right:20px;" id="media-items">
					<?php
						if ( ! $bucket ) { ?>
							<h3 class="media-title"><?php esc_html_e( 'Select a Bucket', 'ea-wc-amazon-s3' ); ?></h3>
							<?php
							...
							if ( is_array( $buckets ) ) {
								...
								echo '<tr>';
								echo '<th>' . esc_html__( 'Bucket name', 'ea-wc-amazon-s3' ) . '</th>';
								echo '<th>' . esc_html__( 'Actions', 'ea-wc-amazon-s3' ) . ' </th>';
								echo '</tr>';
								...
								foreach ( $buckets as $key => $bucket ) {
									...
									echo '<td>' . $bucket . '</td>';
									echo '<td>';
									echo '<a href = "' . esc_url( add_query_arg( 'bucket', $bucket ) ) . '">' . esc_html__( 'Browse', 'ea-wc-amazon-s3' ) . '</a>';
									echo '</td>';
									echo '</tr>';
								}
								echo '</table>';
							...
							if ( is_array( $files ) ) {
								...
								echo '<p><button class="button-secondary" onclick="history.back();">' . esc_html__( 'Go Back', 'ea-wc-amazon-s3' ) . '</button></p>';

								echo '<table class="wp-list-table widefat fixed striped" style="max-height: 500px;overflow-y:scroll;">';

								echo '<tr>';
								echo '<th>' . esc_html__( 'File name', 'ea-wc-amazon-s3' ) . '</th>';
								echo '<th>' . esc_html__( 'File size', 'ea-wc-amazon-s3' ) . '</th>';
								echo '<th>' . esc_html__( 'Uploaded on', 'ea-wc-amazon-s3' ) . '</th>';
								echo '<th>' . esc_html__( 'Actions', 'ea-wc-amazon-s3' ) . '</th>';
								echo '</tr>';
								...
								foreach ( $files as $key => $file ) {
									...
									echo '<td>' . human_time_diff( $file[ 'time' ] ) . ' ' . esc_html__( 'ago', 'ea-wc-amazon-s3' ) . '</td>';

									echo '<td>';
									echo '<a class="insert-s3 button-secondary" href="#" data-s3="' . esc_attr( $file[ 'name' ] ) . '">' . esc_html__( 'Use File', 'ea-wc-amazon-s3' ) . '</a>';
									echo '</td>';
									echo '</tr>';
									$i ++;
								}
							...
							if ( isset( $_GET[ 'p' ] ) && $_GET[ 'p' ] > 1 ) {
								echo '<a class="page-numbers prev button-secondary" href="' . esc_url( remove_query_arg( 'p', $base ) ) . '">' . esc_html__( 'Start Over', 'ea-wc-amazon-s3' ) . '</a>';
							}
							if ( $i >= 10 ) {
								echo '<a class="page-numbers next button-secondary" href="' . esc_url( add_query_arg( array(
									                                                                                      'p'     => $page + 1,
									                                                                                      'start' => $last_file
								                                                                                      ), $base ) ) . '">' . esc_html__( 'View More', 'ea-wc-amazon-s3' ) . '</a>';
							}
							echo '</div>';
							echo '</div>';
						}
			...
			public function media_tabs( $tabs ) {
				$tabs[ 'ea_wc_amazon_buckets' ] = esc_html__( 'Browse S3 Buckets', 'ea-wc-amazon-s3' );
				$tabs[ 'ea_wc_amazon_upload' ]  = esc_html__( 'Upload to Amazon S3', 'ea-wc-amazon-s3' );
			...
			public function upload_iframe_content( $type = 'file', $errors = NULL, $id = NULL ) {
				...
				<script>
					jQuery(document).ready(function ($) {
						$('.ea-wc-amazon-s3-insert').on('click', function (e) {
							e.preventDefault();
				...
				<div class="wrap">
					<form enctype="multipart/form-data" method="post" action="<?php echo esc_url( admin_url() ); ?>">
						<p>
							...
							<label for="ea_wc_amazon_s3_bucket"><?php esc_html_e( 'Select a bucket to upload the file to', 'ea-wc-amazon-s3' ); ?></label>
						</p>
						<p>
						...
						<p>
							<input type="submit" name="ea_wc_amazon_s3_upload_submit" class="button-secondary"
							       value="<?php esc_attr_e( 'Upload to S3', 'ea-wc-amazon-s3' ); ?>"/>
						</p>
						<?php wp_nonce_field( 'ea_wc_amazon_s3_upload_file' ); ?>
						<?php
							if ( ! empty( $_GET[ 'ea_wc_amazon_s3_success' ] ) && '1' == $_GET[ 'ea_wc_amazon_s3_success' ] ) {
								echo '<div class="ea_wc_amazon_s3_errors"><p class="ea_wc_amazon_s3_success">' . __( 'Success! <a href="#" class="ea-wc-amazon-s3-insert">Insert uploaded file</a>.' ) . '</p></div>';
							}
						?>
					</form>
				</div>
			...
			public function add_tab( $settings_tabs ) {
				$settings_tabs[ 'ea-wc-amazon-s3' ] = esc_html__( 'Amazon S3 Settings', 'ea-wc-amazon-s3' );
			...
			public function settings_options() {
				...
				$settings[] = array(
					'title' => esc_html__( 'Amazon S3 Settings', 'ea-wc-amazon-s3' ),
					'type'  => 'title',
					'id'    => 'ea_wc_amazon_s3',
				);

				$settings[] = array(
					'name' => esc_html__( 'Amazon S3 Access Key ID', 'ea-wc-amazon-s3' ),
					'desc' => esc_html__( 'After logging into your Amazon S3 account, click on "Security Credentials" in the sidebar. Scroll down to "Access Keys (Access Key ID and Secret Access Key)" and you will see your Access Key ID. Copy and paste it here.', 'ea-wc-amazon-s3' ),
					'id'   => 'ea_wc_amazon_s3_key',
					'type' => 'text',
				);

				$settings[] = array(
					'name' => esc_html__( 'Amazon S3 Secret Key', 'ea-wc-amazon-s3' ),
					'desc' => esc_html__( 'In the same Access Credentials area, your "Secret Key" will be hidden. You will need to click the "Show" link to see it. Copy and paste it here.', 'ea-wc-amazon-s3' ),
					'id'   => 'ea_wc_amazon_s3_secret_key',
					'type' => 'text',
				);

				$settings[] = array(
					'name' => esc_html__( 'Amazon S3 EndPoint', 'ea-wc-amazon-s3' ),
